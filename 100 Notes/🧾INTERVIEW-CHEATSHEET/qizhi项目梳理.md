...
#### 告警模块的背景是什么？
在堡垒机平台中需要支持，若系统中的服务或中间件如audit，core，es，redis等组件宕机。在系统页面提示出现红色告警信息，同时配置支持邮件和终端通知。
#### 怎么实现的？
1. 数据收集
	1. 考虑使用时间序列数据库，（重写入/重读取），prometheus
	2. 模型选择：推/拉
		1. 拉：指标收集器，定期从运行的应用中拉取指标。使用 http 请求。（prometheus），
		2. 推：指标来源直接发给指标收集器，
2. 数据传输
	- **Prometheus/Alertmanager**：判断服务挂了，触发告警。
	- **告警接入服务（后端）**：收到 Webhook → `PUBLISH alert_channel "core down"`。
	- **告警推送服务（后端另一个进程/线程）**：`SUBSCRIBE alert_channel`，收到消息后 → `ws.send(...)` 给所有在线的前端。
	- **前端**：监听 WebSocket 消息 → 更新 UI（显示红条）。
3. 数据存储
4. 告警
5. 可视化
	1. 前端红条
...
* **这里邮件发送是怎么做的？**
  * 邮件发送集成了SMTP协议以及邮件发送组件，SpringBoot提供了JAvaMailSender，在配置文件中配置一下发送参数，springboot就会自动创建一个JavaMailSender的Bean；再定一个一个邮件通知服务，在告警的方法里使用一下即可。
